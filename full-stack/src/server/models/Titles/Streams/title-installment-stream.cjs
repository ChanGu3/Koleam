...
class TitleInstallmentStream extends ModelExtension {
    ...
    static async #CreateDirectory(titleStream) {
        try {
    ...
    static async #RenameDirectory(oldTitleStream, newTitleStream) {
        try {
    ...
    static async #DeleteDirectory(titleStream) {
        try {
            const dirName = this.#TitleStreamDirPath(titleStream)
    ...
    static async RewriteMediaMasterFile(streamID) {
        try {
            ...
            throw new Error(`could not rewrite master playlist --- ${err.message}`)
        }
    }

    static async AddToDB(titleID, installmentID, label, streamNumber, synopsis, releaseDate) {
        try {
    ...
    static async UpdateInDB(id, { label = null, synopsis = null, releaseDate = null, streamNumber = null } = {}, transaction = null) {
    ...
    static async GetByID(id, includeMediaData = false, transaction = null) {
        try {
            if (await this.#Exists(id)) {
                const default_query = {
                    where: {
                ...
                return title_installment_stream_data
            } else {
                throw new Error(`could not get ${TitleInstallmentStream.name} with id:${id}`)
            }
        } catch (err) {
    ...
    static async GetAll({ limit = 10, offset = 0, installmentID = null, isStreamNumberDesc = false }, transaction = null) {
        try {
            // -----------------
            // -----------------
            ...
            return original_title_installment_data_list.map((element, _index) => {
