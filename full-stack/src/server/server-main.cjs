...
const PUBLIC_PORT = process.env.PUBLIC_PORT || 5775
const LOCAL_PORT = process.env.LOCAL_PORT || 5776
...
const sessionSecretKey = "some-key" // TODO: GENERATED ENVIRONMENT VARIABLE, WHEN CHANGED WILL HAVE TO RESET ALL SESSIONS IN DATABASE
...
function LocalAppSetup() {
    ...
    //
    local_app.use(cookieParser(sessionSecretKey))
    local_app.use(
        session({
...
function PublicAppSetup() {
    ...
    // - Initalizes Cookies Setup -
    //
    public_app.use(cookieParser(sessionSecretKey))
    public_app.use(
        session({
            secret: sessionSecretKey,
            resave: false,
