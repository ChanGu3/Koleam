...
class TitleInstallment extends ModelExtension {
    ...
    static async #CreateDirectory(titleInstallment) {
        try {
    ...
    static async #DeleteDirectory(titleInstallment) {
        try {
            const dirName = this.#TitleDirPath(titleInstallment)
    ...
    static async AddToDB(titleID, label, isSeason) {
        try {
    ...
    static async UpdateInDB(id, { label = null, installmentNumber = null, isSeason = null } = {}, transaction = null) {
        try {
            ...
            if (installmentNumber !== null && installmentNumber !== undefined) {
                if (installmentNumber < 0) {
                    throw new Error(`installmentNumber must be a non-negative number`)
                }
    ...
    static async GetByID(id) {
        try {
            if (await this.#Exists(id)) {
                ...
                const { createdAt: _c1, updatedAt: _u1, ...rest } = installment.toJSON()
                return rest
            } else {
                throw new Error(`could not get ${TitleInstallment.name} with id: ${id}`)
            }
        } catch (err) {
    ...
    static async GetAll({ titleID = null, installmentNum = null, limit = 10, offset = 0 } = {}, transaction = null) {
        try {
            const query = {
                limit: limit,
                offset: offset,
            ...
            if (titleID && installmentNum) {
                ...
                const { createdAt: _c1, updatedAt: _u1, ...rest } = installmentList.toJSON()
                return [rest]
            } else {
                installmentList = await TitleInstallment.findAll(query)
                return installmentList.map((element) => {
                    const { createdAt: _c1, updatedAt: _u1, ...rest } = element.toJSON()
                    return rest
                })
            }
        } catch (err) {
